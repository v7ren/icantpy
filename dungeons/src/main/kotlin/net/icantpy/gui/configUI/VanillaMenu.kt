...
internal enum class ConfigTab {
    ...
    fun caption(): String = when (this) {
        TIMERS -> "HUD timers"
        ALERTS -> "Clock flashes"
        WAYPOINTS -> "Walk-on cmds"
        LEAP -> "Spirit Leap"
        RENAME -> "Item names"
        LOOK -> "Layout & theme"
        STATS -> "Armor swap"
...
internal class MenuDraw(
    ...
    fun paintConfigChrome(m: ConfigMetrics, tab: ConfigTab, onSelect: (ConfigTab) -> Unit) {
        ...
        unclipped {
            ...
            when (chrome.layout) {
                GuiLayoutId.RAIL -> {
                    ...
                    brand(m)
                    text(tab.caption(), m.panelX + m.sidebar + 16, m.panelY + 18, palette.secondary, descFont)
                ...
                GuiLayoutId.RIBBON -> {
                    ...
                    brand(m)
                    text(tab.caption(), m.panelX + m.panelW - measure(tab.caption(), descFont) - 16, m.panelY + 16, palette.secondary, descFont)
                    var tabX = m.panelX + 16
                    val tabY = m.panelY + m.header
                    ConfigTab.entries.forEach { item ->
                        val w = measure(item.label(), buttonFont) + 18
                        navTab(item.label(), tabX, tabY, w, m.tabBar, tab == item) { onSelect(item) }
                        tabX += w + 4
                    }
                }
                GuiLayoutId.COMPACT -> {

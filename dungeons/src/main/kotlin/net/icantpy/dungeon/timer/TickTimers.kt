...
import net.icantpy.gui.config.GuiLookSettings
import net.icantpy.gui.config.HudOverlaySettings
...
import net.icantpy.slayer.SlayerCarrySettings
import net.icantpy.qol.stats.StatsArmorSettings
...
object TickTimers {
    ...
    val bossActive: Boolean
        get() = clocks.phaseElapsed.isNotEmpty() ||
            clocks.necron >= 0 ||
            clocks.goldorTick >= 0 ||
            clocks.goldorStart >= 0 ||
            clocks.maxorElapsed >= 0 ||
            clocks.pad >= 0 ||
            clocks.lightning >= 0 ||
            clocks.py >= 0 ||
            clocks.stormTick >= 0
    ...
    fun updateLeap(persist: Boolean = true, transform: (LeapOrientSettings) -> LeapOrientSettings) {
    ...
    fun updateDropUltimate(transform: (DropUltimateSettings) -> DropUltimateSettings) {
        settings = settings.copy(dropUltimate = transform(settings.dropUltimate))
        persist()
    }

    fun updateSlayer(transform: (SlayerCarrySettings) -> SlayerCarrySettings) {
        settings = settings.copy(slayer = transform(settings.slayer))
        persist()
    }

    /** Adds a transient on-screen notification using the alert HUD. */
    fun pushNotification(text: String, durationTicks: Int = 60) {
        if (text.isBlank()) return
        val next = notifications.toMutableList()
        next.removeAll { it.text == text }
        next += ActiveNotification(text, durationTicks.coerceAtLeast(1))
        notifications = next
    }
    ...
    fun onBossBar(name: String, progress: Float) {
        clocks = clocks.onBossBar(Text.strip(name), progress)
    }
    ...
    fun tick() {
        ...
        if (DungeonListener.floor.name in setOf("F7", "M7") && dungeon.inBoss) {
            clocks = clocks.onGoldorSection(LeapGameState.leapSection())
        }
        fireNotifications()
    }
    ...
    fun hudPieces(preview: Boolean = false): List<HudPiece> {
        val dungeon = DungeonListener.state
        val debug = settings.debugMode
        val pieces = ArrayList<HudPiece>(8)
        if (settings.carryHud && settings.slayer.enabled) {
            val (x, y) = settings.position(TimerHud.CARRY)
            pieces += HudPiece(TimerHud.CARRY, carryHudText(preview), x, y)
        }
        if (settings.dungeonHud) {
            dungeonTimerText(preview)?.let { text ->
                val (x, y) = settings.position(TimerHud.DUNGEON)
                pieces += HudPiece(TimerHud.DUNGEON, text, x, y)
            }
        }
    ...
    private fun dungeonTimerText(preview: Boolean): String? {
        val lines = TimerClock.elapsedClocks
            .sortedBy { it.ordinal }
            .mapNotNull { clock ->
                val time = dungeonClockValue(clock)
                val shown = if (time >= 0) time else if (preview) 200 else return@mapNotNull null
                val max = when (clock) {
                    ...
                    else -> TickClocks.GOLDOR_ELAPSED_MAX
                }
                TickClocks.format(
                    "§f${clock.label()}:",
                    shown,
    ...
    private fun dungeonClockValue(clock: TimerClock): Int = when (clock) {
        ...
        else -> clocks.phaseElapsed[clock] ?: TickClocks.INACTIVE
    }

    private fun carryHudText(preview: Boolean): String {
        val slayer = settings.slayer
        val players = slayer.players
        if (players.isEmpty()) return "Name: 0/20"
        val selected = slayer.selectedPlayer()
        return players.joinToString("\n") { player ->
            val count = if (preview && player.count == 0) 0 else player.count
            val progress = if (player.hasTarget) "$count/${player.target}" else "$count"
            val marker = if (player === selected) " <" else ""
            "${player.name}: $progress$marker"
        }
    }

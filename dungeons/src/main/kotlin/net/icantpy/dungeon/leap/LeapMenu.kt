...
object LeapMenu {
    ...
    fun clickDebug(teammates: List<LeapPlayer>, screenW: Int, screenH: Int, mouseX: Int, mouseY: Int): Boolean {
        val player = playerAt(teammates, screenW, screenH, mouseX, mouseY) ?: return false
        leapTo(player, null)
        return true
    }
    ...
    fun leapTo(player: LeapPlayer, menu: AbstractContainerMenu?) {
        ...
        if (menu != null) {
            val index = validatedSlotIndex(menu, player.name) ?: return
    ...
    private fun playerAt(
        ...
        val cfg = settings()
        val center = centerPlayer(teammates)
        val index = LeapMenuLayout.hit(screenW, screenH, mouseX, mouseY, cfg, center != null) ?: return null
        return if (index == 4) center else teammates.getOrNull(index)
    }
    ...
    fun validatedSlotIndex(menu: AbstractContainerMenu, name: String): Int? =
        containerHeadSlots(menu).firstOrNull { slot ->
            headName(slot.item.hoverName.string)?.equals(name, ignoreCase = true) == true
        }?.index
